...
export async function POST(request: NextRequest) {
  try {
    ...
    const checkoutUrl = invoice.payment?.gateway?.data?.checkout_url;
    ...
    if (!checkoutUrl) {
      console.error('Missing checkout URL in response:', invoiceResponse);
      return NextResponse.json(
        { error: 'Failed to get checkout URL from Sell.app' },
        { status: 500 }
      );
    }
